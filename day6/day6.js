...
my correct answer: 38017587
*/
  let input = await files.readAFile("input.txt");
  let total = 0;
  let lines = input.split(`\n`);
  let times = lines[0].split(":")[1].trim().split(/[\s]+/);
  let dists = lines[1].split(":")[1].trim().split(/[\s]+/);
  ...
  for (let j = 0; j < times.length; j++) {
    let wins = 0;
    for (let i = 0; i < times[j]; i++) {
      let obj = {
        hold: i,
        speed: i,
        time: times[j] - i,
        dist: i * (times[j] - i),
        win: i * (times[j] - i) > dists[j],
      };
      if (obj.win == true) wins++;
      raceResult[j].push(obj);
    }
    winCount.push(wins);
  }
  // console.log(times, dists);
  // console.log(raceResult, winCount);
  // console.log(winCount[0] * winCount[1] * winCount[2] * winCount[3]);

  times = lines[0].split(":")[1].trim().replaceAll(/[\s]+/g, "");
  dists = lines[1].split(":")[1].trim().replaceAll(/[\s]+/g, "");
  let bigRaceResult = [];
  //  let winCount = [];
  //for (let j = 0; j < 1; j++) {
  let wins = 0;
